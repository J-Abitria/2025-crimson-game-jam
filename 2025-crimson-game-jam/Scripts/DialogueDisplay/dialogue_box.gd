...
func run_response_dialogue(response_dialogue: ResponseDialogue) -> void:
	...
	for entry in response_dialogue.responses:
		var button: Button = Button.new()
		var callable = Callable(self, "select_response").bind(entry)
		button.text = entry.response
		button.pressed.connect(callable)
		option_container.add_child(button)
	self.set_process_input(false)

func select_response(choice: ResponseOption) -> void:
	for child in self.option_container.get_children():
		child.queue_free()
	dialogue.set_state(choice.state_transition_index)
	proceed_dialogue()
